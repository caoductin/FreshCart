...
class PaymentViewModel: ObservableObject
{
    ...
    func serviceCallList(){
        ServiceCall.post(parameter: [:], path: Globs.SV_PAYMENT, isToken: true ) { responseObj in
    ...
    func serviceCallRemove(pObj: PaymentModel){
        ServiceCall.post(parameter: ["pay_id": pObj.id ], path: Globs.SV_REMOVE_PAYMENT, isToken: true ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: KKey.status) as? String ?? "" == "1" {
    ...
    func serviceCallAdd(didDone: ((  )->())? ) {
        ...
        ServiceCall.post(parameter: ["name":  txtName, "card_number": txtCardNumber, "card_month": txtCardMonth, "card_year": txtCardYear  ], path: Globs.SV_ADD_PAYMENT, isToken: true ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: KKey.status) as? String ?? "" == "1" {
                    self.clearAll()
                    self.serviceCallList()
